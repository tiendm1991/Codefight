...
public class Level12 {
  ...
  int differentSquares(int[][] matrix) {
    ...
    for (int i = 0; i < row - 1; i++) {
      for (int j = 0; j < col - 1; j++) {
        lsSubSquare.add(matrix[i][j] + "" + matrix[i][j + 1] + "" + matrix[i + 1][j] + ""
            + matrix[i + 1][j + 1]);
      }
    }
  ...
  boolean checkSudoku(int[] arrCheck) {
    for (int i = 0; i < arrCheck.length - 1; i++) {
      if (arrCheck[i] < 0 || arrCheck[i] > 9)
        return false;
      for (int j = i + 1; j < arrCheck.length; j++) {
        if (arrCheck[j] < 0 || arrCheck[j] > 9 || arrCheck[i] == arrCheck[j])
          return false;
      }
    }
    return true;
