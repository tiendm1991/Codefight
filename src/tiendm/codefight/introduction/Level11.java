...
public class Level11 {
  ...
  int deleteDigit(int n) {
    ...
    return Integer.parseInt(
        s.substring(0, mark) + (mark + 1 < s.length() ? s.substring(mark + 1, s.length()) : ""));
  }
  ...
  // int mark = 0;
  // for(int i = log; i > 0; i--){
  // if((int)(n % Math.pow(10, i+1) / Math.pow(10, i)) < (int)(n %
  ...
  // ((int)((n % Math.pow(10, mark)) / Math.pow(10, log-mark)) * Math.pow(10,
  // log-mark))+ n%10);
  // }
