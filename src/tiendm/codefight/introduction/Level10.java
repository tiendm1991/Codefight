...
public class Level10 {
  String buildPalindrome(String s) {
    ...
    while (center < s.length() - 1) {
      ...
      if (strAddMax.indexOf(sCheck.toString()) < strAddMax.lastIndexOf(sCheck.toString()))
        break;
      center++;
    }
  ...
  boolean isValidNumber(String s, boolean n, boolean c, boolean d) {
    ...
    if (c) {
      if (!s.contains(","))
        ...
        if (arrSplitCommma[0].length() > 3)
          return false;
        for (int i = 1; i < arrSplitCommma.length; i++) {
          if (arrSplitCommma[i].length() != 3)
            return false;
        }
      }
    }
  ...
  boolean isMAC48Address(String inputString) {
    if (!checkRange(inputString.charAt(inputString.length() - 1)))
      return false;
  ...
  boolean checkIp(String s) {
    if (s.length() != 2)
      return false;
    if (checkRange(s.charAt(0)) && (checkRange(s.charAt(1)))) {
      return true;
    }
    return false;
  }
