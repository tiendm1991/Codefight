...
public class MultiplyToSingleDigit {
  ...
  private boolean isDangerous(int number, int added, int limit, List<Integer> lsResult,
      List<Integer> lsFalse) {
    String s = "";
    if (check(lsResult, number))
      return true;
    if (check(lsFalse, number))
      return false;
    while (true) {
      s = number + "";
      int product = 1;
      ...
      if (product < 10 || lsFalse.contains(product))
        return false;
      if (product == number || product > limit || lsResult.contains(product))
        return true;
      number = product;
    }
  }
