...
public class MirrorLake {
	...
	int constructSquare(String s) {
		...
		for(int i = 0; i < 26; i++){
			if(occ[i] > 0) lsCharracter.add(occ[i]);
		}
		...
		for(int i = max-1 ; i >= min; i--) {
			...
			if(square * square == i && getnum(i).equals(sCheck)) return i;
		}
		return -1;
	}
	...
	int numberOfClans(int[] divisors, int k) {
		...
		for(int i = 0; i < k-1; i++){
			...
			clanI.add(i+1);
			for(int j = i+1; j < k; j++){
				if(checkFriend(divisors, i+1, j+1)) clanI.add(j+1);
			}
	...
	boolean checkMerge(int i1, int i2, List<ArrayList<Integer>> clans, boolean[] visited) {
		...
		for(int i : clanLess){
			if(!clanGreat.contains(i)) return false;
		}
		visited[visit] = true;
		return true;
	}
