...
public class ShortOutpost {
  ...
  boolean boxesPacking(int[] length, int[] width, int[] height) {
    for (int i = 0; i < length.length - 1; i++) {
      for (int j = i + 1; j < length.length; j++) {
        if (!checkInside(length[i], width[i], height[i], length[j], width[j], height[j]))
          return false;
      }
    }
    return true;
  ...
  boolean rowsRearranging(int[][] matrix) {
    ...
    for (int i = 0; i < matrix[0].length; i++) {
      ...
      if (strCol[i].equals(" "))
        return false;
      if (i > 0 && !strCol[i].equals(strCol[i - 1]))
        return false;
    }
    return true;
  }
  ...
  public static void main(String[] args) {
    ...
    int[] x =
        {13098, 1308, 12398, 52433, 213, 424, 213, 243, 12213, 54234, 99487, 81892, 11111, 97897};
