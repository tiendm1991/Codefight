...
public class TimeRiver {
	...
	int[] videoPart(String part, String total) {
		...
		int lTotal = Integer.parseInt(arrTotal[0])*3600 + Integer.parseInt(arrTotal[1])*60 + Integer.parseInt(arrTotal[2]);
		String[] arrPart = part.split("\\:");
		int lPart = Integer.parseInt(arrPart[0])*3600 + Integer.parseInt(arrPart[1])*60 + Integer.parseInt(arrPart[2]);
		int x = gcd(lTotal,lPart);
		return new int[]{lPart/x, lTotal/x};
	}

	int gcd(int lTotal, int lPart) {
		if(lTotal % lPart == 0) return lPart;
	...
	int newYearCelebrations(String takeOffTime, int[] minutes) {
		...
		while(i < minutes.length){
			...
			if(time <= timeCheck && timeStop >= timeCheck) count++;
			time = timeStop - 60;
			i++;
		}
		if(time <= timeCheck) count++;
		return count;
	} 
	...
	int missedClasses(int year, int[] daysOfTheWeek, String[] holidays) {
		...
		SimpleDateFormat fomat = new SimpleDateFormat("MM-dd-yyyy");
		Calendar cal = Calendar.getInstance();
		for(int i : daysOfTheWeek){ sCheck += (i == 7) ? 1 : i+1; }
	...
	int holiday(int x, String weekDay, String month, int yearNumber) {
		...
		if(cal.get(Calendar.MONTH)!= iMonth) return -1;
		return dayMonth;
	}
