...
public class TournamentDay15 {
  ...
  /*
   * static int maximumSum(int[] A, int[][] Q) { class Helper { Helper() { }
   * 
   * int arraySum(int[] A, int l, int h) { int result = 0; for (int i = l; i <= h; i++) { result +=
   * A[i]; } return result; }
   * 
   * int arrayMaxSum(int[] A, int range) { int i = A.length - 1; Arrays.sort(A); return arraySum(A,
   * i - range, i); } }
   * 
   * Helper h = new Helper(); int maxSum = 0; for (int rows = 0; rows < Q.length; rows++) { int
   * arraySum = h.arrayMaxSum(A, Q[rows][Q[rows].length - 1] - Q[rows][0]); if (arraySum > maxSum) {
   * maxSum = arraySum; } }
   * 
   * return maxSum; }
   */
  ...
  public static void main(String[] args) {
    ...
    // System.out.println(maximumSum(new int[]{2, 1, 2}, new int[][]{{0,
    // 1}}));
    // System.out.println(maximumSum(new int[]{4, 2, 1, 6, 5, 7, 2, 4}, new
    // int[][]{{1, 6}, {2, 4}, {3, 6}, {0, 7}, {3, 6}, {4, 4}, {5, 6}, {5,
    // 6}, {0, 1}, {3, 4}}));
