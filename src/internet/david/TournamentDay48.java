...
public class TournamentDay48 {
  ...
  private static int countVisibleTowerPairs(int[] position, int[] height) {
    ...
    for (int i = 0; i < position.length; i++) {
      for (int j = i + 1; j < position.length; j++) {
        int A = height[j] - height[i], B = position[i] - position[j],
        ...
        for (int k = 0; k < position.length; k++) {
          if (leftBound < position[k] && (A * position[k] + B * height[k] + C) * C < 0) {
            visible = false;
          }
        }
  ...
  private static int growingPlant(int upSpeed, int downSpeed, int desiredHeight) {
    ...
    int currentHeight = 0, dayIndex = 1;
  ...
  private static int chipMoving(int[][] grid) {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < m; j++) {
        if (i < n - 1) {
          dp[i + 1][j][0] = Math.min(Math.min(dp[i + 1][j][0], dp[i][j][0] + grid[i + 1][j]),
              dp[i][j][1] + grid[i + 1][j] + 10);
        }
        if (j < m - 1) {
          dp[i][j + 1][1] = Math.min(Math.min(dp[i][j + 1][1], dp[i][j][1] + grid[i][j + 1]),
  ...
  private static int balancedBinaryTreeDiameter(int n) {
    ...
    return h.maxDepth((n - 1) / 2) + h.maxDepth(n / 2);
  }
  ...
  private static int countLuckyNumbers(int n) {
    ...
    /*
     * array dp (short for dynamic programming) is used for storing the interim values.
     */
  ...
  public static void main(String[] args) {
    // System.out.println(mergeKArrays(new int[][]{{1, 3, 5}, {2, 3}, {2, 3,
    // 5, 8}}));
    // System.out.println(threeGlasses(new int[]{16, 5, 3}));
    // System.out.println(isGeometricProgression(new int[]{2, 4, 8, 17,
    // 34}));
    // System.out.println(chipMoving(new int[][]{{0, 0, 99, 99, 99}, {99, 0,
    // 0, 0, 99}, {99, 99, 99, 0, 99}, {99, 99, 99, 0, 99}, {99, 99, 99, 0,
    // 0}}));
    /*
     * System.out.println(chipMoving(new int[][]{{4, 11, 6, 16, 6, 9, 3, 2, 10, 14}, {1, 7, 8, 11,
     * 16, 3, 18, 13, 5, 12}, {10, 12, 10, 2, 15, 0, 18, 11, 13, 7}, {14, 15, 15, 15, 9, 5, 11, 11,
     * 7, 4}, {5, 9, 15, 19, 4, 4, 11, 4, 14, 8}, {5, 5, 15, 2, 15, 15, 14, 10, 19, 19}, {8, 9, 2,
     * 3, 18, 12, 15, 10, 10, 3}, {8, 2, 1, 11, 4, 3, 16, 7, 8, 13}, {17, 2, 17, 10, 17, 13, 16, 14,
     * 8, 5}, {10, 4, 9, 3, 13, 10, 5, 4, 3, 10}}));
     */
    // System.out.println(chipMoving(new int[][]{{19, 11, 13, 0, 8, 1, 18,
    // 12}}));
