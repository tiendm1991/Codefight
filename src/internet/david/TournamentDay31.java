...
public class TournamentDay31 {
  ...
  private static int minMaxDifference(int[] inputArray) {
    ...
    int indexOfMinimum = 0, indexOfMaximum = 0;
  ...
  private static int nightRoute(int[][] city) {
    ...
    for (int count = 0; count < len - 1; count++) {
      ...
      for (int v = 0; v < len; v++)
        ...
        if (!sptSet[v] && city[u][v] != -1 && dist[u] != Integer.MAX_VALUE
  ...
  private static boolean match(String str1, String str2) {
    /*
     * String prefixStr2 = prefix(str2); if (str1.equals(prefixStr2)) return true; else { int diff =
     * 0; for (char ch : str1.toCharArray()) if (!prefixStr2.contains(ch)) diff++; return diff == 1;
     * 
     * }
     */
    return false;
  ...
  public static void main(String[] args) {
    ...
    // System.out.println(nightRoute(new int[][]{{-1, 5, 20}, {21, -1, 10},
    // {-1, 1, -1}}));
    System.out.println(
        closestLocation("Cat", new int[][] {{-2, 0}, {1, 2}, {2, 1, 2, 4}, {-3, -1, 4, -1}},
