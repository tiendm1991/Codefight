...
public class TournamentDay6 {
  ...
  static int[][] neighboringCells(int[][] matrix) {
    ...
    class Helper {
      ...
      int neighbors(int[][] matrix, int row, int col) {
        ...
        return getElement(matrix, row, col + 1) + getElement(matrix, row, col - 1)
            + getElement(matrix, row + 1, col) + getElement(matrix, row - 1, col);
      }
    }
  ...
  public static void main(String[] args) {
    ...
    // System.out.println(maxSubmatrixSum(new int[][]{{1, 12, 11, 10}, {4,
    // 3, 2, 9}, {5, 6, 7, 8}}, 2, 1));
    // System.out.println(dfsComponentSize(new boolean[][]{{false, true,
    // false}, {true, false, false}, {false, false, false}}, 0));
    // System.out.println(neighboringCells(new int[][]{{0, 0, 0}, {0, 0, 0},
    // {0, 0, 0}}));
