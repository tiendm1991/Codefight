...
public class TournamentDay35 {
  ...
  private static int arrayMinimumAboveBound(int[] inputArray, int bound) {
    ...
    for (int anInputArray : inputArray) {
      if (anInputArray > bound && (anInputArray < best)) {
        best = anInputArray;
      }
    }
  ...
  private static int countPathsThroughCell(int n, int m, int x, int y) {
    ...
    /*
     * // Create a 2D table to store results of subproblems int[][] count = new int[m][n];
     * 
     * // Count of paths to reach any cell in first column is 1 for (int i = x; i < n; i++)
     * count[i][0] = 1;
     * 
     * // Count of paths to reach any cell in first column is 1 for (int j = y; j < m; j++)
     * count[0][j] = 1;
     * 
     * // Calculate count of paths for other cells in bottom-up manner using // the recursive
     * solution for (int i = 1; i < n; i++) { for (int j = 1; j < m; j++)
     * 
     * // By uncommenting the last part the code calculatest he total // possible paths if the
     * diagonal Movements are allowed count[i][j] = count[i - 1][j] + count[i][j - 1]; //+
     * count[i-1][j-1];
     * 
     * } return count[n - 1][m - 1];
     */
    /*
     * int p = 1; for (int i = 0; i < Math.min(n - x - 1, m - y - 1); p /= ++i) p *= (n - x + m - y
     * - i); return p;
     */
  ...
  public static void main(String[] args) {
    // System.out.println(arrayMinimumAboveBound(new int[]{1, 3, 5, 4, 2,
    // 6}, 3));
