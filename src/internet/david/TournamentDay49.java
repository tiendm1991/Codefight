...
public class TournamentDay49 {
        ...
        for (int k = j + 1; k < table[0].length; ++k) {
          for (int jj = 0; jj < table[0].length; ++jj) {
            for (int kk = jj + 1; kk < table[0].length; ++kk) {
              if (table[i][j].equals(table[i + 1][kk]) && table[i][k].equals(table[i + 1][jj]))
                r++;
  ...
  private static int logicalOrder(String current, String first, String second) {
    ...
    while (!first.isEmpty()
      ...
      if (current.charAt(current.length() - 1) != first.charAt(first.length() - 1))
        r++;
    ...
    if (first.compareTo(current) < 0 && current.compareTo(second) < 0)
      return r;
    ...
    for (int i = 0; i < current.length(); ++i) {
      ...
      if (tmp.compareTo(first) > 0 && tmp.compareTo(second) < 0)
  ...
  private static int aquariumFish(int[] w, int[][][] f, int t) {
    /*
     * def aquariumFish(w, f, t): a = [] for i in range(len(f)): l = (f[i][0][0] ** 2 + f[i][0][1]
     * ** 2) ** .5 if l == 0: l = 1 a += [[w[i], f[i][1], [f[i][0][0] / l, f[i][0][1] / l]]]
     * 
     * while True: minT = t + 1 for A in a: for B in a: curt = t + 1 if A[2][0] != B[2][0]: curt =
     * (A[1][0] - B[1][0]) * 1.0 / (B[2][0] - A[2][0]) elif A[2][1] != B[2][1]: curt = (A[1][1] -
     * B[1][1]) * 1.0 / (B[2][1] - A[2][1])
     * 
     * x1 = A[1][0] + A[2][0] * curt y1 = A[1][1] + A[2][1] * curt x2 = B[1][0] + B[2][0] * curt y2
     * = B[1][1] + B[2][1] * curt
     * 
     * if abs(x1 - x2) < 1e-6 > abs(y1 - y2): if curt > -1.0e-6 and curt < minT: minT = curt l =
     * a.index(A) r = a.index(B)
     * 
     * if minT > t: break
     * 
     * for i in range(len(a)): a[i][1][0] += a[i][2][0] * minT a[i][1][1] += a[i][2][1] * minT
     * 
     * if l > r: l, r = r, l
     * 
     * if a[l][0] >= a[r][0]: a[l][0] += a[r][0] del a[r] else: a[r][0] += a[l][0] del a[l]
     * 
     * t -= minT
     * 
     * return len(a)
  ...
  public static void main(String[] args) {
    ...
    System.out.println(topTeamsTable(new String[][] {{"Canada", "Sweden", "Czech", "Finland"},
        {"Canada", "Sweden", "Slovakia", "Czech"}, {"Canada", "Sweden", "Czech", "Slovakia"},
        {"Sweden", "Canada", "Czech", "Finland"}, {"Sweden", "Czech", "Slovakia", "Canada"}}));
    System.out.println(logicalOrder("fullb", "filea", "filew"));
    System.out.println(cropCircles(new int[][] {{-2, 0}, {9, 4}},
        new int[][] {{0, 0, 4}, {-2, 0, 1}, {1, 1, 1}, {9, 1, 4}}));
    System.out.println(aquariumFish(new int[] {2, 3, 2, 1},
        new int[][][] {{{1, 2}, {-1, 0}}, {{1, 0}, {-3, 1}}, {{-1, 2}, {1, 0}}, {{0, 1}, {2, -4}}},
        5));
  }
