...
public class TournamentDay5 {
  ...
  /*
   * static int chessBoardSquaresUnderQueenAttackR(int a, int b) { return a * b *
   * chessBoardSquaresUnderQueenAttack2(a, b) + Math.abs(a - b) * 2; }
   */

  /*
   * static int chessBoardSquaresUnderQueenAttack(int a, int b) { int[][] chessBoard = new
   * int[a][b];
   * 
   * int sum = 0; for (int k = 0; k < a * b; k++) { // clear the matrix for (int i = 0; i < a; i++)
   * { for (int j = 0; j < b; j++) { chessBoard[i][j] = 0; } }
   * 
   * for (int i = 0; i < a; i++) { for (int j = 0; j < b; j++) { if ((i == k && j != k) || (i != k
   * && j == k)) { if (chessBoard[i][j] == 0) { chessBoard[i][j] = 1; sum++; } } if (i > 0 && j > 0)
   * { if (k + j < a && k + j < b) { if (chessBoard[k + j][k + j] == 0) { chessBoard[k + j][k + j] =
   * 1; sum++; } } if (k - j > 0 && k - j > 0 && k - j < b) { if (chessBoard[k - j][k - j] == 0) {
   * chessBoard[k - j][k - j] = 1; sum++; } } if (k + j < a && k - j > 0 && k - j < b) { if
   * (chessBoard[k + j][k - j] == 0) { chessBoard[k + j][k - j] = 1; sum++; } } if (k - j > 0 && k +
   * j < b && k - j < b) { if (chessBoard[k - j][k + j] == 0) { chessBoard[k - j][k + j] = 1; sum++;
   * } } } } } }
   * 
   * return sum; }
   */
  ...
  static int chessBoardSquaresUnderQueenAttack(int a, int b) {
    ...
    for (int i = 0; i < a; i++) {
      for (int j = 0; j < b; j++) {
        for (int x = 0; x < a; x++) {
          for (int y = 0; y < b; y++) {
            if (i == x || y == j || Math.abs(i - x) == Math.abs(j - y))
              ret++;
          }
        }
      }
  ...
  static int alphabeticShiftEncoding(String inputString, String encodedString) {
    ...
    class Helper {
      ...
      int commonChars(String s1, String s2) {
        ...
        return h1.size();
        /*
         * Character[] res = h1.toArray(new Character[0]); return res.length;
         */
      }
  ...
  public static void main(String[] args) {
    ...
    // System.out.println(imagesOverlapping(new boolean[][]{{false, true,
    // false}, {true, false, true}}, new boolean[][]{{true, false}}));
