...
public class TournamentDay22 {
  ...
  /*
   * static int[] borderSums(int[][] matrix) { class Helper { int sum;
   * 
   * int getSum() { return sum; }
   * 
   * int[][] borderSum(int[][] matrix) { int len = matrix.length, newRow = 0, newCol; int[][]
   * resMatrix = new int[len / 2][len / 2]; sum = 0; for (int i = 0; i < len; i++) { newCol = 0; for
   * (int j = 0; j < len; j++) { if (i == 0 || j == 0 || i == len - 1 || j == len - 1) { sum +=
   * matrix[i][j]; } else { resMatrix[newRow][newCol] = matrix[i][j]; newCol++; } } if (i != 0 && i
   * != len - 1) newRow++; } return resMatrix; } }
   * 
   * Helper h = new Helper(); int[] result = new int[matrix.length / 2]; int i = 0; while
   * (matrix.length != 1) { matrix = h.borderSum(matrix); result[i] += h.getSum(); i++; } return
   * result; }
   */
  ...
  static String digitalNumber(int k) {
    ...
    /*
     * dp[i][j] - the largest number that is obtainable from j segments and that has i as a
    ...
    for (int i = 1; i < M; i++) {
      for (int j = 0; j < 3; j++) {
        ...
        for (int d = 0; d < 10; d++) {
          ...
          int cur = dp[pSum][pSegmCnt] == 0 ? d : Integer.parseInt(d + "" + dp[pSum][pSegmCnt]);
  ...
  public static void main(String[] args) {
    ...
    // System.out.println(borderSums(new int[][]{{1, 2, 2, 3}, {0, 1, 0, 2},
    // {4, -1, -1, -3}, {4, -1, -1, 3}}));
    // System.out.println(borderSums(new int[][]{{1, 1, 1, 1, 1, 1}, {1, 1,
