...
public class TournamentDay43 {
  ...
  private static int[] splitByValue(int k, int[] elements) {
    ...
    return Stream.concat(less.stream(), grtr.stream()).collect(Collectors.toList()).stream()
  ...
  private static int lastDigit(int a, int b) {
    /*
     * int[][] scale = new int[][]{{0, 0, 0, 0}, {1, 1, 1, 1}, {2, 4, 6, 8}, {3, 9, 7, 1}, {4, 6, 4,
     * 6}, {5, 5, 5, 5}, {6, 6, 6, 6}, {7, 9, 3, 1}, {8, 4, 2, 6}, {9, 1, 9, 1}}; return
     * scale[a][b];
     */
    if (b == 0)
      return 1;
  ...
  private static int circleGrid(double X, double Y, double R) {
    /*
     * int a = 0; for (double i = Math.floor(X - R); i <= Math.ceil(X + R); i++) for (double j =
     * Math.floor(Y - R); j <= Math.ceil(Y + R); j++) if ((X - i) * (X - i) + (Y - j) * (Y - j) <= R
     * * R) a++; return a;
     */
    int a = 0;
  ...
  private static ArrayList<Integer> nextPermutation(ArrayList<Integer> permutation) {
    ...
    for (int i = permutation.size() - 2; i >= 0; i--) {
      if (permutation.get(i) < permutation.get(i + 1)) {
        int index = i + 1, tmp = permutation.get(i + 1);
        for (int j = i + 1; j < permutation.size(); j++) {
          if (permutation.get(j) < permutation.get(i)
              && permutation.get(j) < permutation.get(index)) {
            index = j;
          }
        }
   ...
   *//*
     * int n = a.length - 1; // Number of bits to represent int int INT_BITS = 32; // Initialize
     * index of chosen elements int index = 0;
     * 
     * // Traverse through all bits of integer starting from // the most significant bit (MSB) for
     * (int i = INT_BITS - 1; i >= 0; i--) { // Initialize index of maximum element and the maximum
     * element int maxInd = index, maxEle = Integer.MIN_VALUE; for (int j = index; j < n; j++) { //
     * If i'th bit of set[j] is set and set[j] is greater // than max so far. if ((a[j] & (1 << i))
     * != 0 && a[j] > maxEle) { maxEle = a[j]; maxInd = j; } }
     * 
     * // If there was no element with i'th bit set, move to smaller i if (maxEle ==
     * Integer.MIN_VALUE) continue;
     * 
     * // Put maximum element with i'th bit set at index 'index' int tmp = a[index]; a[index] =
     * a[maxInd]; a[maxInd] = tmp; // swap(a[index], a[maxInd]);
     * 
     * // Update maxInd and increment index maxInd = index;
     * 
     * // Do XOR of set[maxIndex] with all numbers having i'th // bit as set. for (int j = 0; j < n;
     * j++) { // XOR set[maxInd] those numbers which have the i'th // bit set if ((j != maxInd) &&
     * ((a[j] & (1 << i)) != 0)) a[j] = a[j] ^ a[maxInd]; }
     * 
     * // Increment index of chosen elements index++; }
     * 
     * // Final result is XOR of all elements int res = 0; for (int i = 0; i < n; i++) res ^= a[i];
     * return res;
     *//*
       * int ans = Integer.MIN_VALUE; // Initialize result
       * 
       * // Pick starting points of subarrays for (int i = 0; i < a.length; i++) { int curr_xor = 0;
       * // to store xor of current subarray
       * 
       * // Pick ending points of subarrays starting with i for (int j = i; j < a.length; j++) {
       * curr_xor = curr_xor ^ a[j]; ans = Math.max(ans, curr_xor); } } return ans; }
       */
  ...
  public static void main(String[] args) {
    // System.out.println(Arrays.toString(fractionSum(new int[]{3, 5}, new
    // int[]{7, 5})));
    // System.out.println(isLowerTriangularMatrix(new int[][]{{1, 0, 0}, {4,
    // 0, 0}, {0, 3, 3}}));
    // System.out.println(isLowerTriangularMatrix(new int[][]{{0, -1, 0},
    // {0, 0, 0}, {0, 0, 0}}));
    // System.out.println(Arrays.toString(splitByValue(5, new int[]{1, 3, 5,
    // 7, 6, 4, 2})));
    ...
    /*
     * ArrayList<Integer> arr = new ArrayList<>(); arr.add(1); arr.add(3); arr.add(2); arr.add(4);
     * System.out.println(nextPermutation(arr).toString());
     */
    // System.out.println(divNumber(3, 2, 49));
    // System.out.println(matrixElementsSum(new int[][]{{0, 1, 1, 2}, {0, 5,
    // 0, 0}, {2, 0, 3, 3}}));
    // System.out.println(matrixElementsSum(new int[][]{{1, 1, 1, 0}, {0, 5,
    // 0, 1}, {2, 1, 3, 10}}));
    // System.out.println(equationSolutions(1, 4));
    // System.out.println(twoPolygons(new int[][]{{0, 0}, {1, 0}, {0, 2}},
    // new int[][]{{2, 0}, {2, 2}, {0, 2}}));
