...
public class TournamentDay42 {
  ...
  private static int alphabeticShiftEncoding(String inputString, String encodedString) {
    ...
    for (int i = 0; i < inputString.length(); i++) {
      int mistakes = 0, k = encodedString.charAt(i) - inputString.charAt(i);
      if (k < 0) {
        k += 26;
      }
      for (int j = 0; j < inputString.length(); j++) {
        if ((inputString.charAt(j) - 'a' + k) % 26 != encodedString.charAt(j) - 'a') {
          mistakes++;
        }
      }
  ...
  public static void main(String[] args) {
    // System.out.println(Arrays.toString(repeatedSubstring("abacabadabacaba",
    // 6)));
    // System.out.println(Arrays.toString(repeatedSubstring2("abacabadabacaba",
    // 6)));
    ...
    // System.out.println(differentDigitsNumberSearch(new int[]{22, 111,
    // 101, 124, 33, 30}));
