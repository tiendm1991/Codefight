...
public class TournamentDay16 {
  ...
  static int swapNeighbouringDigits(int n) {
    ...
    while (n != 0) {
      int digit1 = n % 10, digit2 = ((n - digit1) / 10) % 10;
   ...
   * ArrayList<ArrayList<Integer>> matrix = new ArrayList<>(); int answer = 0; int ballsRemaining =
   * 0; int colorIdx = 0; int row = 0; while (colorIdx < colors) { ArrayList<Integer> tmp = new
   * ArrayList<>(); for (int j = 0; j < boxSize; j++) { if (ballsRemaining >= ballsPerColor) {
   * colorIdx++; ballsRemaining = 1; } else { ballsRemaining++; } tmp.add(j, colorIdx == colors ?
   * colorIdx - 1 : colorIdx); // matrix[row][j] = colorIdx == colors ? colorIdx - 1 : colorIdx; if
   * (j - 1 > 0 && tmp.get(j - 1) != tmp.get(j)) answer++; } matrix.add(tmp); row++; }
   * 
   * 
   *//*
     * for (int i = 0; i < matrix.length; i++) for (int j = 1; j < matrix[0].length; j++) if
     * (matrix[i][j] != matrix[i][j - 1]) answer++;
     *//*
       * 
       * return answer; }
       */
