...
public class TournamentDay51 {
  ...
  private static char[][] snakeGame(char[][] gameBoard, String commands) {
    ...
    int snakeLength = -1;
    int[] curDir;
    char[][] directionChars = new char[][] {{' ', '^', ' '}, {'<', ' ', '>'}, {' ', 'v', ' '}};
  ...
  private static boolean reverseToSort(ArrayList<Integer> inputArray) {
    ...
    for (int i = 0; i < inputArray.size(); i++) {
      for (int j = i + 1; j <= inputArray.size(); j++) {
        ArrayList<Integer> middle = new ArrayList<>(), result = new ArrayList<>();
  ...
  private static boolean isCorrectSentence(String inputString) {
    if ('A' <= inputString.charAt(0) && 'Z' >= inputString.charAt(0)
  ...
  public static void main(String[] args) {
    ...
    /*
     * ArrayList<Integer> input = new ArrayList<>(); input.add(-1); input.add(5); input.add(4);
     * input.add(3); input.add(2); input.add(8); System.out.println(reverseToSort(input));
     */
