...
public class TournamentDay10 {
  ...
  static String smartAssigning(String[][] information) {
    String[] available = new String[] {"", "1", String.valueOf(Integer.MAX_VALUE),
  ...
  /*
   * static String[] recurringTask(String firstDate, int k, String[] daysOfTheWeek, int n) { final
   * List<String> daysInWeek = new ArrayList<>( Arrays.asList(new String[] { "Monday", "Tuesday",
   * "Wednesday", "Thursday", "Friday", "Saturday", "Sunday" }) ); final int[] days = new int[] {31,
   * 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
   * 
   * class Helper { boolean isLeap(int year) { return year % 4 == 0 && year % 100 != 0 || year % 400
   * == 0; } int countLeap(int yearLeft, int yearRight) { int result = 0; for (int i = yearLeft; i
   * <= yearRight; i++) { if (isLeap(i)) { result++; } } return result; } int getDaysInMonth(int
   * month, int year) { int result = days[month]; if (isLeap(year) && month == 1) { result++; }
   * return result; } boolean daysContainsIdx(int[] days, int idx) { for (int day : days) if (idx ==
   * day) return true; return false; } }
   * 
   * ArrayList<String> recurringDays = new ArrayList<>();
   * 
   * int weekDayInt = Integer.parseInt(firstDate.substring(0, 2)) - 1; int monthInt =
   * Integer.parseInt(firstDate.substring(3, 5)) - 1; int yearNumber =
   * Integer.parseInt(firstDate.substring(6, 10));
   * 
   * Helper h = new Helper(); int leaps = h.countLeap(2015, yearNumber); int newWeekDay =
   * (daysInWeek.indexOf("Thursday") + leaps * 366 + (yearNumber - 2015 - leaps) * 365) % 7; for
   * (int i = 0; i < monthInt; i++) { newWeekDay = (newWeekDay + h.getDaysInMonth(i, yearNumber)) %
   * 7; } int[] daysOfTheWeekIdx = new int[daysOfTheWeek.length]; for (int i = 0; i <
   * daysOfTheWeek.length; i++) { daysOfTheWeekIdx[i] = daysInWeek.indexOf(daysOfTheWeek[i]); }
   * 
   * recurringDays.add(firstDate); n--; boolean nextDate = true; int daysCount = weekDayInt + 1; int
   * daysInMonth = h.getDaysInMonth(monthInt, yearNumber); while (n > 0) { daysCount++; newWeekDay =
   * (newWeekDay + 1) % 7; if (nextDate && h.daysContainsIdx(daysOfTheWeekIdx, newWeekDay)) {
   * recurringDays.add(String.format("%02d", daysCount) + "/" + String.format("%02d", monthInt + 1)
   * + "/" + yearNumber); n--; nextDate = false; } if (h.daysContainsIdx(daysOfTheWeekIdx,
   * newWeekDay) && daysCount >= k * 7) { n--; recurringDays.add(String.format("%02d", daysCount) +
   * "/" + String.format("%02d", monthInt+ 1) + "/" + yearNumber); }
   * 
   * if (daysCount > daysInMonth) { monthInt++; daysInMonth = h.getDaysInMonth(monthInt,
   * yearNumber); daysCount = 0; } }
   * 
   * String[] result = new String[recurringDays.size()]; for (int i = 0; i < recurringDays.size();
   * i++) { result[i] = recurringDays.get(i); } return result; }
   */
  static String[] recurringTask(String firstDate, int k, String[] daysOfTheWeek, int n) {
    final List<String> daysInWeek = new ArrayList<>(Arrays.asList(new String[] {"Monday", "Tuesday",
    ...
    int newWeekDay =
        (daysInWeek.indexOf("Tuesday") + leaps * 366 + (yearNumber - 1991 - leaps) * 365) % 7;
    ...
    while (num > 0) {
      ...
      if (nextDate && h.daysContainsIdx(daysOfTheWeekIdx, newWeekDay)) {
        recurringDays.add(String.format("%02d", daysCount) + "/"
            + String.format("%02d", monthInt + 1) + "/" + yearNumber);
        num--;
        nextDate = false;
      } else if (h.daysContainsIdx(daysOfTheWeekIdx, newWeekDay) && diffCount > period) {
        num--;
        recurringDays.add(String.format("%02d", daysCount) + "/"
            + String.format("%02d", monthInt + 1) + "/" + yearNumber);
      }
  ...
  public static void main(String[] args) {
    ...
    // String[] recurringTask = recurringTask("01/01/2015", 2, new
    // String[]{"Monday", "Thursday"}, 4);
    // String[] recurringTask = recurringTask("22/02/2020", 1, new
    // String[]{"Saturday"}, 2);
    String[] recurringTask =
        recurringTask("23/02/2000", 2, new String[] {"Wednesday", "Friday"}, 4);
