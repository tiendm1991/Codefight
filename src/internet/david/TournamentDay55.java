...
public class TournamentDay55 {
  ...
  private static int trapRooms(final char[][] museum) {
    ...
    class Helper {
      ...
      boolean dfs(int x, int y) {
        if (x < 0 || x >= museum.length || y < 0 || y >= museum[0].length) {
          return true;
        }
        if (visited[x][y]) {
          return answer[x][y];
  ...
  public static void main(String[] args) {
    // System.out.println(closestSequence(new int[]{1, 2, 6}, new int[]{0,
    // 1, 3, 4, 5}));
    // System.out.println(Arrays.toString(theJanitor("abacaba")));
    /*
     * System.out.println(trapRooms(new char[][]{{'U', 'L'}, {'R', 'L'}}));
     */
    // System.out.println(dotProduct(new int[]{1, 1, 1}, new int[]{0, 1,
    // -1}));
    // System.out.println(mySubstring("abcde", 2, 3));
    // System.out.println(mySubstring("abcdef", 0, 2));
    // System.out.println(polygonPerimeter(new boolean[][]{{false, true,
