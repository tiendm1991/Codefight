...
public class TournamentDay8 {
  ...
  static int holiday(int x, String weekDay, String month, int yearNumber) {
    final List<String> months =
        new ArrayList<String>(Arrays.asList(new String[] {"January", "February", "March", "April",
            "May", "June", "July", "August", "September", "October", "November", "December"}));
    final List<String> daysInWeek = new ArrayList<String>(Arrays.asList(new String[] {"Monday",
    ...
    int newWeekDay =
        (daysInWeek.indexOf("Thursday") + leaps * 366 + (yearNumber - 2015 - leaps) * 365) % 7;
  ...
  /*
   * static int minCoins(int coins[], int m, int V) { // base case if (V == 0) return 0;
   * 
   * // Initialize result int res = Integer.MAX_VALUE;
   * 
   * // Try every coin that has smaller value than V for (int i=0; i<m; i++) { if (coins[i] <= V) {
   * int sub_res = minCoins(coins, m, V-coins[i]); // Check for INT_MAX to avoid overflow and see if
   * // result can minimized if (sub_res != Integer.MAX_VALUE && sub_res + 1 < res) res = sub_res +
   * 1; } } return res; }
   */
  ...
  /*
   * static int countTriangles(int[] X, int[] Y) { class Helper { Helper() { }
   * 
   * int[] point(int x, int y) { return new int[]{x, y}; }
   * 
   * int fact(int n) { if (n == 0) return 1; return n * fact(n - 1); } boolean collinear(int[] p1,
   * int[] p2, int[] p3) { return (p1[1] - p2[1]) * (p1[0] - p3[0]) == (p1[1] - p3[1]) * (p1[0] -
   * p2[0]); // return (y1 - y2) * (x1 - x3) == (y1 - y3) * (x1 - x2); } } Helper h = new Helper();
   * HashSet<int[]> hashset = new HashSet<>(); for (int i = 0; i < X.length; i++) {
   * hashset.add(h.point(X[i], Y[i])); } int pointNumber = hashset.size(); HashSet<int[]>
   * substractNumber = new HashSet<>(); for (int[] p1 : hashset) { for (int[] p2 : hashset) { for
   * (int[] p3 : hashset) { if (h.collinear(p1, p2, p3)) { substractNumber.add(p1);
   * substractNumber.add(p2); substractNumber.add(p3); } } }
   * 
   * } return pointNumber == 3 ? 1 : (h.fact(pointNumber) / (h.fact(3) * h.fact(pointNumber - 3)) -
   * 0); }
   */
  ...
  static int josephusProblem(int n, int k) {
    /*
     * List<Integer> p = new LinkedList<Integer>(); for (int i = 0; i < n; i++) p.add(i+1);
     * List<Integer> r = new LinkedList<Integer>(); int i = (1 - 2) % n; for (int j = n; j > 0; j--)
     * { i = (i + 1) % n--; r.add(p.remove(i--)); } return r.get(r.size() - 1);
     */
    int removeIdx = 0;
  ...
  public static void main(String[] args) {
    ...
    // System.out.println(countTriangles(new int[]{0, 0, 1, 1}, new int[]{0,
    // 1, 1, 0}));
    // System.out.println(countTriangles(new int[]{0, -1, -2}, new int[]{0,
    // -2, -4}));
    // System.out.println(countTriangles(new int[]{0, 0, 0, 0}, new int[]{1,
    // 2, 3, 4}));
